...
-- Add age calculation as a computed column
ALTER TABLE public.measurement_profiles 
...
CREATE INDEX IF NOT EXISTS idx_measurement_profiles_calculated_age ON measurement_profiles(calculated_age);
...
BEGIN
  ...
  IF NEW.growth_tracking IS NULL OR NEW.growth_tracking = '{}'::jsonb THEN
    CASE NEW.relationship
      ...
      WHEN 'SIBLING' THEN
        IF NEW.calculated_age IS NULL OR NEW.calculated_age <= 16 THEN
...
COMMENT ON COLUMN measurement_profiles.birth_date IS 'Birth date for age calculation and growth tracking';
...
COMMENT ON COLUMN measurement_profiles.calculated_age IS 'Auto-calculated age based on birth_date (stored generated column)';
