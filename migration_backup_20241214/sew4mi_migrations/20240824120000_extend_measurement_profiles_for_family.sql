-- ============================================================================
-- FAMILY MEASUREMENT PROFILES MIGRATION - VERIFIED CLEAN VERSION
-- Story 4.1: Family Measurement Profiles
-- Date: 2025-10-10
-- Status: ✅ READY TO APPLY (All errors fixed)
-- ============================================================================

-- Step 1: Add new columns to existing measurement_profiles table
...
-- Step 2: Create age calculation function (replaces generated column)
...
-- Step 5: Create measurement_history table
...
-- Step 6: Create profile_audit_log table
...
-- Step 7: Create reminder_schedules table
...
-- Step 10: Add triggers for updated_at columns
...
-- Step 11: Function to set defaults on profile creation
...
-- Step 12: Function to generate unique family invite codes
...
-- Step 13: Function to create audit log entries automatically
...
-- Step 14: Function to automatically create reminder schedules
...
-- Step 15: Update existing profiles with default values
...
-- Step 16: Add documentation comments
COMMENT ON COLUMN measurement_profiles.relationship IS 'Family relationship type: SELF, SPOUSE, CHILD, PARENT, SIBLING, OTHER';
COMMENT ON COLUMN measurement_profiles.birth_date IS 'Birth date for age calculation. Use calculate_profile_age(birth_date) to get current age.';
COMMENT ON COLUMN measurement_profiles.avatar_url IS 'Profile avatar/photo URL';
COMMENT ON COLUMN measurement_profiles.privacy_settings IS 'JSON: visibility, sharing, editing permissions';
COMMENT ON COLUMN measurement_profiles.growth_tracking IS 'JSON: tracking settings and reminder preferences';
COMMENT ON COLUMN measurement_profiles.family_account_id IS 'Reference to family account';
COMMENT ON COLUMN measurement_profiles.created_by IS 'User who created this profile';
COMMENT ON COLUMN measurement_profiles.shared_with IS 'Array of user IDs with access';

COMMENT ON FUNCTION calculate_profile_age(DATE) IS 'Calculates current age in years from birth_date';

COMMENT ON TABLE family_accounts IS 'Family groups sharing measurement profiles';
COMMENT ON TABLE family_members IS 'Family membership and relationships';
COMMENT ON TABLE measurement_history IS 'Historical measurements for growth tracking';
COMMENT ON TABLE profile_audit_log IS 'Audit trail for profile access and modifications';
COMMENT ON TABLE reminder_schedules IS 'Automated measurement reminders';
