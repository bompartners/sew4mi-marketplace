...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'IN_PROGRESS' AND enumtypid = 'dispute_status'::regtype) THEN
        ALTER TYPE dispute_status ADD VALUE IF NOT EXISTS 'IN_PROGRESS' AFTER 'OPEN';
    END IF;
END$$;
...
CREATE POLICY "Users can view evidence for their disputes" ON dispute_evidence
    FOR SELECT USING (
        EXISTS (
            ...
            AND (o.customer_id = auth.uid() OR o.tailor_id = auth.uid() OR d.raised_by = auth.uid())
        )
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE sla_deadline < NOW() AND status IN ('OPEN', 'IN_PROGRESS')) as overdue_disputes,
